...
/**
 * Removes near-black backgrounds connected to image edges.
 * Useful for sprite sheets where each frame sits on black tiles.
 */
export function removeBlackBackground(img, threshold = 42) {
  const canvas = document.createElement('canvas');
  canvas.width = img.naturalWidth || img.width;
  canvas.height = img.naturalHeight || img.height;
  const W = canvas.width, H = canvas.height;
  ...
  const queue = [];
  function seed(pos) {
    if (!removed[pos] && isNearBlack(pos * 4, threshold)) {
      removed[pos] = 1;
      queue.push(pos);
    }
  }
  ...
  while (qi < queue.length) {
    const pos = queue[qi++];
    const x = pos % W, y = Math.floor(pos / W);
    if (x > 0)     { const nb = pos - 1; if (!removed[nb] && isNearBlack(nb * 4, threshold)) { removed[nb] = 1; queue.push(nb); } }
    if (x < W - 1) { const nb = pos + 1; if (!removed[nb] && isNearBlack(nb * 4, threshold)) { removed[nb] = 1; queue.push(nb); } }
    if (y > 0)     { const nb = pos - W; if (!removed[nb] && isNearBlack(nb * 4, threshold)) { removed[nb] = 1; queue.push(nb); } }
    if (y < H - 1) { const nb = pos + W; if (!removed[nb] && isNearBlack(nb * 4, threshold)) { removed[nb] = 1; queue.push(nb); } }
  }
  ...
  // Soften dark fringe attached to removed pixels.
  for (let i = 0; i < W * H; i++) {
    ...
    if (r < 85 && g < 85 && b < 85) {
      ...
      if (hasRemovedNeighbor) {
        data[i * 4 + 3] = Math.round(data[i * 4 + 3] * 0.35);
      }
    }
  }
