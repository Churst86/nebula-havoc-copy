...
export function updateBossTier1Fire(e, p, s, sounds) {
  ...
  if (e.fireTimer <= 0) {
    ...
    // 5-way rapid spread (increased from 3)
    for (let i = -2; i <= 2; i++) {
      const a = baseAngle + i * 0.20;
      s.enemyBullets.push({ x: e.x, y: e.y, vx: Math.cos(a) * 5.5, vy: Math.sin(a) * 5.5, boss: true });
    }
    e.fireTimer = 5; // reduced from 8 — fires more often
  }

  e._specialTimer = (e._specialTimer || 90) - 1; // reduced from 180
  if (e._specialTimer <= 0) {
    ...
    e._specialTimer = 90; // reduced from 180
  }
}
