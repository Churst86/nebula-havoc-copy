...
export function spawnBoss(W, wave, hpMult) {
  const bossHp = Math.round((50 + wave * 12.5) * hpMult * 2);
...
export function updateBossTier1Fire(e, p, s, sounds) {
  ...
  if (e.fireTimer <= 0) {
    ...
    acquireBullet(s, { x: e.x, y: e.y, vx: Math.cos(a) * 5.5, vy: Math.sin(a) * 5.5, boss: true }, 'enemy');
...
// Tier 3 (Wave 15) — Beholder Boss is handled via beholderLogic.js
...
export function updateBossTier3Fire(e, p, s, W, H, spawnExplosion, sounds, onScoreChange, BLOCK_SIZE, getBlockCells) {
  // Beholder-specific logic is called from GameCanvas
}
...
function fireBoss5Weapon(gun, e, p, s) {
  ...
  if (gun === 'spread') {
    for (let i = 0; i < 11; i++) {
      ...
      acquireBullet(s, { x: e.x, y: e.y, vx: Math.sin(rad) * 5, vy: Math.cos(rad) * 5, boss: true }, 'enemy');
    }
  } else if (gun === 'laser') {
    for (let li = 0; li < 5; li++) {
      acquireBullet(s, { x: e.x, y: e.y + li * 5, vx: (dx / len) * 8, vy: (dy / len) * 8, boss: true }, 'enemy');
    }
  } else if (gun === 'photon') {
    acquireBullet(s, { x: e.x, y: e.y, vx: (dx / len) * 2.5, vy: (dy / len) * 2.5, boss: true, big: true, photonOrb: true, orbSize: 18 }, 'enemy');
  } else if (gun === 'bounce') {
    for (let i = 0; i < 5; i++) {
      const ang = (i / 5) * Math.PI * 2;
      acquireBullet(s, { x: e.x, y: e.y, vx: Math.cos(ang) * 4.5, vy: Math.sin(ang) * 4.5, boss: true, bouncing: true, bouncesLeft: 5 }, 'enemy');
    }
  } else if (gun === 'missile') {
    for (let mi = 0; mi < 3; mi++) {
      const spread = (mi - 1) * 0.4;
      acquireBullet(s, { x: e.x, y: e.y, vx: Math.sin(spread) * 3, vy: (dy / len) * 5, boss: true, big: true, homing: true, target: p, homingStrength: 0.15 }, 'enemy');
    }
  } else if (gun === 'shotgun') {
    for (let i = 0; i < 9; i++) {
      const angleDeg = -60 + i * 15;
      const rad = (angleDeg * Math.PI) / 180;
      acquireBullet(s, { x: e.x, y: e.y, vx: Math.sin(rad) * 4.5 + (dx / len) * 2, vy: Math.cos(rad) * 4.5 + (dy / len) * 2, boss: true }, 'enemy');
    }
  }
}
