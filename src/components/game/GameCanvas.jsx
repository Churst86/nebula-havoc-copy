...
export default function GameCanvas({ gameState, setGameState, onScoreChange, onBlockScoreChange, onLivesChange, onMaxLivesChange, onWaveChange, onPowerupChange, onBossWarning, continuesLeft, onContinueUsed, isPaused, difficultyConfig, gameSpeed = 30, carryOverPowerups = null, shopUpgrades = null, startWave = 1, onLoadProgress = null, bossMode = false, mobileSpeed = 1.0, joystickVisible = true, joystickSize = 1.0, motionControlEnabled = false, motionInvertX = false, motionInvertY = false, motionAccelSpeed = 1.0 }) {
  ...
  function spawnWaveLocal(W, s) {
    const result = spawnWave(W, s, difficultyConfig, bossMode, onBossWarning);
    if (result === 'gameover') {
      s.running = false;
  ...
  function drawPowerupItem(ctx, item) {
    ...
    if (item.type === 'star') {
      ...
      drawSprite(ctx, spriteImg, -45, -45, 48, 48);
    } else {
    ...
    s.enemies.forEach(e => {
      if (e.type === 'boss') {
        ...
        if ((e.tier || 1) === 3 && e._shieldActive) {
          // Boss is shielded — no damage
        } else if ((e.tier || 1) === 3 && e._laserHitsPlayer) takeDamage(s);
      } else if (e.type === 'mine') {
          ...
          if (e.hp <= 0) {
            ...
            if (e.type === 'boss') {
              ...
              spawnExplosion(s, e.x, e.y,
                e.type === 'dropper' ? (e.color || '#ffd700') : e.type === 'elite' ? '#ff44ff' : e.type === 'mine' ? '#ff8800' : e.type === 'eater' ? '#44ff88' : '#ff4444',
                e.type === 'mine' ? 30 : e.type === 'eater' ? 20 : 14
              );
            }
            if (e.type === 'dropper') { sounds.killDropper(); s.powerupItems.push({ x: e.x, y: e.y, type: e.dropType, angle: 0 }); }
            if (e.type === 'boss') {
  ...
  return (
    <>
      ...
      <MobileControls keysRef={keysRef} mobileSpeed={mobileSpeed} joystickVisible={joystickVisible} joystickSize={joystickSize} />
    </>
  );
}