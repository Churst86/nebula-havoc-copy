...
export default function GameCanvas({ gameState, setGameState, onScoreChange, onLivesChange, onMaxLivesChange, onWaveChange, onPowerupChange, continuesLeft, onContinueUsed, isPaused, difficultyConfig, gameSpeed = 30, carryOverPowerups = null }) {
  ...
  function drawPlayer(ctx, p, wingmen, shieldHp, enemies, invincibleTimer, keys, starInvincibleTimer, superWingman, superWingmen) {
    ...
    if (playerImage) {
      ...
      ctx.drawImage(playerImage, -32, -32, 60, 60);
    } else {
      ctx.shadowColor = '#00f0ff'; ctx.shadowBlur = 18;
  ...
  function drawPowerupItem(ctx, item) {
    const colors = { spread: '#ffdd00', laser: '#ff44ff', photon: '#44ffaa', wingman: '#44aaff', shield: '#00ccff', bounce: '#aaff00', speed: '#ff8800', rapidfire: '#ff4488', star: '#ffffff', missile: ' rgba(126, 35, 138, 0.84)ff' };
  ...
  const loop = useCallback((timestamp) => {
    ...
    if (wingmanTier > 0) {
      ...
      // Super wingmen: up to 2, flank the player
