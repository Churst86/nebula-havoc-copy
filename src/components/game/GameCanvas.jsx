...
export default function GameCanvas({ gameState, setGameState, onScoreChange, onBlockScoreChange, onLivesChange, onMaxLivesChange, onWaveChange, onPowerupChange, onBossWarning, continuesLeft, onContinueUsed, isPaused, difficultyConfig, gameSpeed = 30, carryOverPowerups = null }) {
  ...
  const loop = useCallback((timestamp) => {
    ...
    if ((s.powerups.laser || 0) > 0) {
      ...
      if (s.laserBeamActive) {
        ...
        ctx.strokeStyle = `rgba(${beamColorRgb},${beamAlpha * 0.4})`; ctx.lineWidth = beamW * 3;
        ...
        ctx.beginPath(); ctx.moveTo(p.x, p.y - 18); ctx.lineTo(p.x, beamEndY); ctx.stroke();
        ctx.strokeStyle = `rgba(${beamCenterRgb},${beamAlpha})`; ctx.lineWidth = beamW * 0.3;
        ctx.beginPath(); ctx.moveTo(p.x, p.y - 18); ctx.lineTo(p.x, beamEndY); ctx.stroke();
        if (s.laserFlareTimer > 0) {
          ...
          ctx.strokeStyle = `rgba(${beamCenterRgb},${flarePct})`; ctx.lineWidth = 2;
